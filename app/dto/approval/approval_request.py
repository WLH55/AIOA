...
class LeaveRequest(BaseModel):
    ...
    start_time: int = Field(..., description="开始时间戳(秒)")
    end_time: int = Field(..., description="结束时间戳(秒)")
    reason: str = Field(..., description="请假原因")
    time_type: int = Field(default=1, description="时长类型(1-小时, 2-天)")
...
class MakeCardRequest(BaseModel):
    ...
    check_type: int = Field(..., description="补卡类型(1-上班卡, 2-下班卡)")
...
class ApprovalRequest(BaseModel):
    ...
    approver_ids: Optional[List[str]] = Field(None, description="审批人ID列表")
    copy_person_ids: Optional[List[str]] = Field(None, description="抄送人ID列表")
    make_card: Optional[MakeCardRequest] = Field(None, description="补卡信息")
    leave: Optional[LeaveRequest] = Field(None, description="请假信息")
    go_out: Optional[GoOutRequest] = Field(None, description="外出信息")
