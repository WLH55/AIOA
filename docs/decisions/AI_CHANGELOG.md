...
### 2026-03-26 Decision #010: 日志轮转机制优化

- **问题**: 原有 `TimedRotatingFileHandler` 仅按时间轮转，日志文件增长到 10GB，启动时轮转超时
- **决策**: 实现"大小+时间"双重轮转机制
  - 单文件超过 50MB 立即轮转
  - 每天午夜自动轮转
  - 旧日志压缩为 `.gz` 格式
- **原因**: 避免 IO 阻塞，确保启动时间不受日志文件大小影响
- **影响**: 日志文件大小可控，启动更稳定
- **替代方案**:
  - 仅按大小轮转：丢失时间维度的日志分组
  - 仅按时间轮转（原有方案）：无法控制单文件大小
- **Files**: `app/config/logger_config.py`
- **Spec**: `docs/specs/003-log-rotation-optimization/`

---

### 2026-03-25 Decision #009: WebSocket 心跳机制优化

- **问题**: 前后端同时发送 ping，无人回复 pong，心跳逻辑混乱
- **决策**: 改为被动心跳模式
  - 前端每 30 秒发送 ping
  - 后端接收 ping 更新心跳时间戳
  - 后端不再主动发送 ping
  - 后端检测超时（60秒未收到心跳）则断开连接
