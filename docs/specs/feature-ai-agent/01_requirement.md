...
#### 7. 传输方式
- **独立 AI 对话页面**：使用 SSE (Server-Sent Events)，HTTP POST + StreamingResponse，单向服务端推送
- **群聊 @AI 场景**：通过 WebSocket 触发，后端内部消费 SSE 流后将事件通过 WS 推送给群成员
- SSE 事件类型：`ai_chunk`、`ai_tool_call`、`ai_tool_result`、`ai_complete`、`ai_error`
- AI 处理通过 `asyncio.create_task` 异步派发，不阻塞正常聊天
...
### AC1: AI 对话核心流程
- 用户可通过 HTTP POST + SSE 流式接口发送自然语言消息给 AI
- AI 通过 Function Calling 自主决策调用工具，返回正确结果
- AI 响应通过 SSE 事件流式推送（ai_chunk → ai_complete）
- 群聊 @AI 场景通过 WebSocket 触发，后端内部桥接 SSE 流后通过 WS 推送
...
- **风险点 4**：SSE 连接中断导致 AI 响应流丢失
  - 缓解：AI 完整响应持久化到 ChatLog，刷新后可查询；客户端断开时后端取消 Agent 任务
